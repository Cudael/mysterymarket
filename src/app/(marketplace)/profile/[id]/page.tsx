...
export async function generateMetadata({
  ...
  return {
    title: isCreator
      ? `${user.name ?? "Creator"} - Creator Profile - MysteryMarket`
      : `${user.name ?? "User"} - MysteryMarket`,
    description:
      user.bio ??
      (isCreator
        ? `Browse high-value ideas by ${user.name ?? "this creator"} on MysteryMarket.`
        : `${user.name ?? "A user"} on MysteryMarket.`),
  };
}
...
export default async function ProfilePage({
  ...
  const isCreator = user.role === "CREATOR" || user.ideas.length > 0;
  ...
  if (!isCreator) {
    // Buyer profile — simple card
    return (
      <div className="container mx-auto px-4 py-12 max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="rounded-[12px] border border-border bg-card p-8 sm:p-10 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
          <div className="flex flex-col items-center gap-6 text-center sm:flex-row sm:items-start sm:text-left">
            ...
            <div className="flex-1">
              ...
              {user.bio && (
                <p className="mt-2 text-[15px] leading-[1.6] text-foreground/70">
                  {user.bio}
                </p>
              )}
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-3 sm:justify-start">
                ...
                <span className="flex items-center gap-2 rounded-[6px] bg-muted border border-border px-3 py-1.5 text-[13px] font-medium text-foreground/60">
                  <UserCircle2 className="h-4 w-4 text-foreground/40" />
                  Buyer on MysteryMarket
                </span>
              </div>
            </div>
          </div>
  ...
  // Creator profile — full view
  const categoryCounts = user.ideas.reduce<Record<string, number>>(
    (acc, idea) => {
      if (idea.category) {
  ...
  const isTopRated =
    avgRating != null && Number(avgRating) >= 4.5 && reviewCount >= 3;
          ...
          <div className="flex-1">
            ...
            {topCategories.length > 0 && (
              <div className="mt-4 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
                <span className="text-[12px] font-semibold uppercase tracking-wider text-foreground/40">
                  Specializes in
                </span>
                {topCategories.map((cat) => (
                  <span
                    key={cat}
                    className="inline-flex items-center rounded-[6px] border border-primary/20 bg-primary/5 px-2.5 py-1 text-[12px] font-medium text-primary"
                  >
                    {cat}
                  </span>
                ))}
