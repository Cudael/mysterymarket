import Link from "next/link";
import { Sparkles } from "lucide-react";
import { CATEGORY_META } from "@/lib/constants";

export function Footer() {
  return (
    <footer className="border-t border-white/8 bg-[hsl(252,32%,3%)] mt-auto">
      <div className="container mx-auto px-6 py-16 lg:px-8 max-w-7xl">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-5 lg:gap-8">
          
          <div className="col-span-1 md:col-span-1">
            <Link href="/" className="flex items-center gap-2.5 group w-fit">
              <div className="flex h-8 w-8 items-center justify-center rounded-[8px] bg-primary/30 border border-primary/40 transition-colors group-hover:bg-primary/40">
                <Sparkles className="h-4 w-4 text-[hsl(var(--gold))]" />
              </div>
              <span className="text-[18px] font-bold tracking-tight text-white">
                MysteryMarket
              </span>
            </Link>
            <p className="mt-5 text-[15px] leading-[1.7] text-white/50 max-w-[280px]">
              Where the best ideas find their audience.
            </p>
          </div>

          <div>
            <h3 className="mb-5 text-[14px] font-bold uppercase tracking-wider text-white/60">
              Platform
            </h3>
            <ul className="space-y-3.5">
              <li><Link href="/ideas" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Explore Ideas</Link></li>
              <li><Link href="/creator" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Creator Studio</Link></li>
              <li><Link href="/my" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">My Library</Link></li>
              <li><Link href="/my/saved" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Saved Ideas</Link></li>
              <li><Link href="/account" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Account</Link></li>
            </ul>
          </div>

          <div>
            <h3 className="mb-5 text-[14px] font-bold uppercase tracking-wider text-white/60">
              Categories
            </h3>
            <ul className="space-y-3.5">
              {Object.values(CATEGORY_META).map((cat) => (
                <li key={cat.slug}>
                  <Link href={`/ideas/category/${cat.slug}`} className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">
                    {cat.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="mb-5 text-[14px] font-bold uppercase tracking-wider text-white/60">
              Company
            </h3>
            <ul className="space-y-3.5">
              <li><Link href="/about" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">About Us</Link></li>
              <li><Link href="/blog" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Blog</Link></li>
              <li><Link href="/contact" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Contact Support</Link></li>
            </ul>
          </div>

          <div>
            <h3 className="mb-5 text-[14px] font-bold uppercase tracking-wider text-white/60">
              Legal
            </h3>
            <ul className="space-y-3.5">
              <li><Link href="/privacy" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Privacy Policy</Link></li>
              <li><Link href="/terms" className="text-[15px] font-medium text-white/50 transition-colors hover:text-[hsl(var(--gold))]">Terms of Service</Link></li>
            </ul>
          </div>
          
        </div>
        
        <div className="mt-16 border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-[14px] text-white/30 font-medium">
            © {new Date().getFullYear()} MysteryMarket. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
  );
}
